...
export default function Focustask() {
  ...
  const taskDuration = (() => {
    ...
    const n = typeof d === "string" ? parseInt(d, 10) : typeof d === "number" ? d : 25;
    return Number.isFinite(n) && n > 0 ? n : 25;
  })();

  return (
    <div className="space-y-8">
      <Link to="/focus"   className="inline-flex items-center gap-2 text-gray-700 hover:text-gray-900 rounded-lg px-2 py-1 transition-colors"
>
      <ArrowLeft size={16} /> 
      ...
      <div className="bg-white rounded-[28px] shadow-xl border border-gray-100 p-6 md:p-8">
        <div className="space-y-1">
          <p className="text-sm text-gray-700"><span className="font-medium">Title:</span> {taskTitle}</p>
          {taskDescription && (
            <p className="text-sm text-gray-700 whitespace-pre-wrap"><span className="font-medium">Description:</span> {taskDescription}</p>
          )}
        </div>
      </div>
