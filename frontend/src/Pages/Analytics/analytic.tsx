...
function AnalyticsTabs({ active, onChange }: { active: AnalyticsTab; onChange: (tab: AnalyticsTab) => void }) {
  ...
  return (
    <div className="flex gap-2 bg-[#F5F2FF] p-1 rounded-full w-full max-w-md">
      <button
        className={`${base} ${
          active === "overview" ? "bg-[#FFC94A] text-black shadow-sm" : "text-gray-500"
        }`}
      ...
      <button
        className={`${base} ${
          active === "level" ? "bg-[#FFC94A] text-black shadow-sm" : "text-gray-500"
        }`}
      ...
      <button
        className={`${base} ${
          active === "analytic" ? "bg-[#FFC94A] text-black shadow-sm" : "text-gray-500"
        }`}
