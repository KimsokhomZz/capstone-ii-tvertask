...
const BADGES = {
    ...
    TASK_HERO: {
        ...
        check: async (userId, userXP) => {
            ...
            const tasks = Number(userXP?.tasks_completed ?? userXP?.task_count ?? 0) || 0;
            if (tasks >= 10) return true;

            // Fallback: analyze XPLog `source === 'task'`
            const count = await XPLog.count({ where: { user_id: userId, source: "task" } });
            return count >= 10;
        }
    },
    ...
    FOCUS_CHAMP: {
        ...
        check: async (userId, userXP) => {
            ...
            const count = await XPLog.count({ where: { user_id: userId, source: "pomodoro-complete" } });
            return count >= 5;
        }
    },
    ...
    LEVEL_ACHIEVER: {
        ...
        check: async (userId, userXP) => {
            const level = Number(userXP?.level ?? userXP?.dataValues?.level ?? 1) || 1;
            return level >= 5;
        }
    },
...
async function persistBadges(userId, badges) {
    ...
    if (UserXP.rawAttributes?.badges) {
        ...
        const isJsonCol = UserXP.rawAttributes.badges.type && UserXP.rawAttributes.badges.type.key === "JSON";
