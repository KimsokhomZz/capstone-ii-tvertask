...
exports.addXP = async ({ userId, amount, source = "xp_add" }) => {
    ...
    if (!amount || amount <= 0) throw new Error("XP amount must be greater than 0");
...
exports.claimPendingXP = async (userId) => {
    ...
    const result = await exports.addXP({ userId, amount: pending, source: "pending-claim" });
...
exports.getStatus = async (userId) => {
    ...
    const progressPercent = Math.round(
        ((xp - calculateLevel(xp).prevLevelTotal) /
            (calculateLevel(xp).nextLevelTotal - calculateLevel(xp).prevLevelTotal)) * 100
    );
